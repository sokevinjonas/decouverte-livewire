    ...
    <form wire:submit.prevent="submit" class="w-[400px] mx-auto py-16">
        ...
        <div class="mb-4">
            <input type="text" wire:model.debounce.500ms="first_name"
                class="w-full border @error('first_name') border-red-500 @enderror" placeholder="Prénom">
            @error('first_name')
        ...
        <div class="mb-4">
            <input type="text" wire:model.debounce.500ms="last_name"
                class="w-full border @error('last_name') border-red-500 @enderror" placeholder="Nom">
            @error('last_name')
                <span class="text-red-500">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="mb-4">
            <input type="password" wire:model.debounce.500ms="password"
                class="w-full border @error('password') border-red-500 @enderror" placeholder="Mot de passe">
            @error('password')
                <span class="text-red-500">{{ $message }}</span>
            @enderror
        </div>
        ...
        @if ($role === 'vendor')
            <div class="mb-4">
                <input type="text" wire:model.debounce.500ms="vat_number"
                    class="w-full border @error('vat_number') border-red-500 @enderror" placeholder="Numéro de TVA">
                @error('vat_number')
                    <span class="text-red-500">{{ $message }}</span>
                @enderror
            </div>
        ...
        <button type="submit"
            class="py-2 px-4 bg-indigo-500 hover:bg-indigo-600 rounded text-white">S'inscrire</button>
    </form>
